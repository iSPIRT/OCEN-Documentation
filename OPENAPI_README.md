...
1. Create a tag in `openapi.yaml`
```yaml
tags:
  - name: Echo
...
2. Create `x-tagGroups` entry. (This allows you to categorize multiple tags in a single group)
    A group can have multiple tags assigned to it. 
```yaml
x-tagGroups:
  - name: Echo
    tags:
